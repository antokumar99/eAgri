...
The repository holds two projects: `Backend/` (Express + MongoDB) and `eAgri/`
(the Expo app). They install and run separately.

### Prerequisites
- Node.js >= 18
- Expo Go on a phone, or an Android emulator / iOS simulator
- MongoDB (Atlas or local) — optional for a first run, see *Trying it without a database*
- Firebase project (chat), Cloudinary account (images), SSLCommerz merchant account (payments), OpenWeatherMap key (weather)

### Installation

```bash
git clone https://github.com/antokumar99/eAgri.git
cd eAgri
cd Backend && npm install && cd ../eAgri && npm install && cd ..
```

### Environment variables

Copy the template and fill it in. `Backend/.env` is gitignored — never commit it.

```bash
cp Backend/.env.example Backend/.env
```

`Backend/.env.example` documents every variable. The ones without defaults are:

| Variable | Notes |
| --- | --- |
| `MONGO_URI` | MongoDB connection string. The server exits at boot if missing. |
| `JWT_SECRET` | Long random string, e.g. `openssl rand -hex 32`. Server exits if missing. |
| `BACKEND_URL` | Public URL of the API. **SSLCommerz calls back to this from its own servers**, so `localhost` will never receive a callback — use a tunnel (ngrok/cloudflared) for sandbox testing. |
| `STORE_ID`, `STORE_PASSWD`, `IS_LIVE` | SSLCommerz credentials. Online payment returns 503 until these are set. |
| `EMAIL_USERNAME`, `EMAIL_PASSWORD` | Gmail address and **app password** for verification emails. |
| `CLOUDINARY_*` | Image uploads. |

The app also needs its own two files:

```bash
cp eAgri/config/env.example.js eAgri/config/env.js   # OpenWeatherMap key
```

Then set your machine's LAN IP in `eAgri/config/apiConfig.js` — a phone cannot
reach `localhost`:

```js
export const API_BASE_URL = "http://192.168.0.103:3000";  // your LAN IP
```
...
cd eAgri   && npm start        # Expo — scan the QR code with Expo Go
```

### Trying it without a database

Starts the real API against a throwaway in-memory MongoDB, seeded with two
accounts and five listings. Nothing is persisted.

```bash
cd Backend && npm run dev:memory
```

Sign in as `rahim@eagri.test` (seller) or `karim@eagri.test` (buyer), password
`password123`.

### Tests

An end-to-end suite covering cart, checkout, rentals, orders, the social feed
